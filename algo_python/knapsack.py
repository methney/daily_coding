    ...
    for i in range(len(cargo)+1): #+1은 짐이 없는 경우의 수(근데 왜 하필 loop문으로?) 
        ...
        for c in range(capacity + 1): # 무게를 kg단위로 나눠서 처리? 
            if i==0 or c==0:
                ...
                print(i,'/',c, cargo[i-1], ' pack[', i, '] = ','max(', cargo[i-1][0], '+', pack[i-1][c-cargo[i-1][1]], ',', pack[i-1][c], ')')

                # 여기가 핵심인데, 이 하위순회문제의 핵심인거지...
                # 보면, 일단 max(이번차수(상자)만큼의가치 + A, 가방에넣은동일한 무게 상자의 가치)
                # A : 이번허용가능한무게(c) - 이번차수상자의 무게를 뺀 남은 무게를 혹시 가방에넣은 상자중에 맞아떨어지는게 있다면 그것의 가치
                pack[i].append( 
                    max( 
